...
void dvbapi_add_ecmpid(int demux_id, ushort caid, ushort ecmpid, ulong provid,int chid,int STREAMpid) {
	...
	for (j = 0; j < info_length; j += descriptor_length + 2) {
		...
		if (descriptor_ca_system_id >> 8 == 0x01) {
			for (u=2; u<descriptor_length; u+=15) { 
				...
				dvbapi_add_ecmpid(demux_id, descriptor_ca_system_id, descriptor_ca_pid, descriptor_ca_provider, 0, STREAMpid);
			}
		} else {
...
int dvbapi_parse_capmt(unsigned char *buffer, unsigned int length, int connfd) {
	...
	for (i = 0; i < MAX_DEMUX; i++) {
		if (demux[i].demux_index == demux_index && demux[i].program_number == program_number && ca_pmt_list_management==3 ) {
			return -1; //same pmt on same demux,if not changed, exit
		}
	}
		...
		if ((caid >> 8) == 0x06) {
			...
			if (demux[demux_id].irdeto_numchids != buffer[5]) {
				...
				for (i=1;i<=buffer[5];i++) {
					dvbapi_add_ecmpid(demux_id, caid, demux[demux_id].ECMpids[demux[demux_id].demux_fd[filter_num].pidindex].ECM_PID, provid, i,0);
				}
	...
	for (i=0;i<MAX_DEMUX;i++) {
		if (demux[i].program_number==er->srvid) {
			...
			for(n=0;n<demux[i].ECMpidcount;n++)
				if(er->pid==demux[i].ECMpids[n].ECM_PID && demux[i].ECMpids[n].descrambler != -1){
					ci=demux[i].ECMpids[n].descrambler;
					break;
				}	
			...
			if(ecmtxt != NULL) {
				...
				fprintf(ecmtxt, "cw0: %s\n", cs_hexdump(1,demux[i].lastcw[0+ci*2],8));
				fprintf(ecmtxt, "cw1: %s\n", cs_hexdump(1,demux[i].lastcw[1+ci*2],8));
				fclose(ecmtxt);
				ecmtxt = NULL;
			}
		}
