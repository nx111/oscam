...
[ -d ${builddir}/image${machine}/PLUGINS/OpenXCAS/oscamCAS ] || mkdir -p ${builddir}/image${machine}/PLUGINS/OpenXCAS/oscamCAS
cp $ROOT/build/.tmp/oscam ${builddir}/image${machine}/PLUGINS/OpenXCAS/oscamCAS/
if [ -x $TOOLCHAIN_STAGE/upx ]; then
	cp $TOOLCHAIN_STAGE/upx $ROOT/build/.tmp/
	$ROOT/build/.tmp/upx ${builddir}/image${machine}/PLUGINS/OpenXCAS/oscamCAS/oscam >/dev/null
fi
